...
int sys_fork()
{
	...
	process[i]->shmem_end = process[i]->exe_end;
...
void get_exe_page(unsigned long u_vaddr, unsigned long k_vaddr)
{
	int file_blocknr;
	int i;

	file_blocknr = 1 + u_vaddr / BLOCK_SIZE;
	for (i = 0; i < 8; file_blocknr++, i++)
		read_inode_block(current->executable, file_blocknr, (char *)(k_vaddr + BLOCK_SIZE * i), BLOCK_SIZE);
}
void schedule()
{
	int pid = 0;
...
void process_init()
{
	...
	process[0]->shmem_end = process[0]->exe_end;
